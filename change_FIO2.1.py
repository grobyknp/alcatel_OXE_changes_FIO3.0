...
Рефакторинг: логика разделена на классы для улучшения читаемости и отладки.
"""
...
class CredentialsManager:
    """Загрузка логина/пароля для АТС из JSON-файла."""
...
class SubscriberUpdater:
    ...
    def __init__(self, cred_manager: CredentialsManager):
    ...
    def update(self, number: str, last_name: str, first_name: str) -> Tuple[bool, str]:
        ...
        # Определяем IP по номеру
        host = self._get_host_by_number(number)

        # Получаем учётные данные
        try:
            login, password = self.cred_manager.get_credentials(host)
        except Exception as e:
    ...
    @staticmethod
    def _get_host_by_number(number: str) -> str:
        """Возвращает IP АТС по номеру (формат 10.<регион>.9.4)."""
        region = number[1:3]
        if region == '99':
            return "10.0.16.130"
        region_int = int(region)
        return f"10.{region_int}.9.4"
...
class GUI:
    ...
    def update_ip(self, *args):
        ...
        if re.match(r'9\d{2}\d{4}$', number):
            host = SubscriberUpdater._get_host_by_number(number)
...
def main():
    setup_logging()
    cred_manager = CredentialsManager(Config.SERVERS_JSON_PATH)
    updater = SubscriberUpdater(cred_manager)
    GUI(updater)
